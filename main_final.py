...
def recognize_faces(frame):
    ...
    if time.time() - last_face_recognition_time < 1.0:  
        return []
...
def show_main_interface(username):
    ...
    ctk.CTkLabel(root, text="Advance Attendance System", font=("Arial", 20, "bold"), text_color="white").pack(pady=10)
    ...
    def update_camera():
        while is_camera_running and cap.isOpened():
            ret, frame = cap.read()
            if ret:
